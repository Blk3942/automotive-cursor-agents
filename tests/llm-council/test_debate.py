...
Comprehensive tests for multi-model debate (default: DeepSeek chat + reasoner).
...
class TestLLMCouncilInitialization:
    ...
    def test_council_default_initialization(self):
        ...
        assert council.claude_config.name == "deepseek-chat"
        assert council.gpt_config.name == "deepseek-reasoner"
...
class TestSecurityDebates:
    ...
    async def test_firmware_update_security(self, mock_llm_council):
        ...
        result = await mock_llm_council.debate(
            ...
            task_type=TaskType.SECURITY_REVIEW,
            rounds=4,
            early_consensus_exit=False,
        )

        # Security reviews should be thorough (fixed rounds when mocks always agree)
        assert result.rounds_completed >= 2

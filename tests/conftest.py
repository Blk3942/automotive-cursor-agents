...
def mock_claude_adapter():
    """Mock primary LLM adapter for testing LLM council."""
    adapter = MagicMock()
    adapter.config = Mock()
    adapter.config.name = "deepseek-chat"
    adapter.config.provider = "deepseek"

    _response = """# Technical Analysis
...
## Action Items
1. Document safety assumptions
2. Run MISRA static analysis on new modules
"""

    adapter.get_completion = AsyncMock(return_value=(_response, 150.0))
    adapter.get_stats = Mock(
        return_value={
            "provider": "deepseek",
            "model": "deepseek-chat",
            "request_count": 0,
...
def mock_gpt_adapter():
    """Mock secondary LLM adapter for testing LLM council."""
    adapter = MagicMock()
    adapter.config = Mock()
    adapter.config.name = "deepseek-reasoner"
    adapter.config.provider = "deepseek"

    _response = """# Performance Analysis
...
## Action Items
1. Profile worst-case ISR latency
2. Add load tests for peak CAN traffic
"""

    adapter.get_completion = AsyncMock(return_value=(_response, 120.0))
    adapter.get_stats = Mock(
        return_value={
            "provider": "deepseek",

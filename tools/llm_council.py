...
two LLMs for critical engineering decisions in automotive software development.

By default both participants use the DeepSeek API (OpenAI-compatible). You can
still pass custom ModelConfig to use Anthropic Claude or Azure OpenAI.
...
# ``python tools/llm_council.py`` — ensure package ``tools`` resolves
_LLMC_REPO_ROOT = Path(__file__).resolve().parent.parent
if str(_LLMC_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_LLMC_REPO_ROOT))
...
class ModelAdapter(ABC):
    ...
    def _extract_usage_counts(self, response: Any) -> Tuple[int, int, int, int]:
        """Extract usage token counts from OpenAI/Anthropic-style responses."""
    ...
    async def get_completion(
        ...
        try:
            ...
            prompt_tokens, completion_tokens, total_tokens, cached_prompt_tokens = \
                self._extract_usage_counts(response)
            if total_tokens == 0 and hasattr(response, "usage"):
                # Anthropic SDK compatibility fallback
...
class DeepSeekAdapter(ModelAdapter):
    ...
    def _get_api_key(self) -> str:
        """Prefer DEEPSEEK_API_KEY, then key saved by ``python -m tools.cursor_setup``."""
        env = os.environ.get(self.config.api_key_env, "").strip()
        if env:
            return env
        if (self.config.api_key_env or "").strip().upper() == "DEEPSEEK_API_KEY":
            try:
                from tools.cursor_config import load_stored_deepseek_key
            except ImportError:
                from cursor_config import load_stored_deepseek_key

            stored = load_stored_deepseek_key()
            if stored:
                return stored
        raise ValueError(
            f"API key not found. Set {self.config.api_key_env}, or run "
            f"`python -m tools.cursor_setup` once to save your DeepSeek key."
        )
    ...
    def client(self):
        if self._client is None:
            ...
            base_url = (
                ...
                or os.environ.get("DEEPSEEK_API_BASE", "https://api.deepseek.com")
            ).rstrip("/")
            self._client = OpenAI(api_key=self._get_api_key(), base_url=base_url)
        return self._client
    ...
    async def get_completion(
        ...
        try:
            ...
            self.logger.debug(
                f"DeepSeek response received in {duration_ms:.0f}ms "
                f"({len(response_text)} chars)"
            )
...
def model_adapter_for_config(
    config: ModelConfig, cache: Optional[ResponseCache]
) -> ModelAdapter:
    """Instantiate the correct adapter for a model configuration."""
    provider = (config.provider or "").lower().replace("_", "-")
    if provider == "anthropic":
        return ClaudeAdapter(config, cache)
    if provider == "deepseek":
        return DeepSeekAdapter(config, cache)
    if provider in ("azure-openai", "azure"):
        return GPTAdapter(config, cache)
    raise ValueError(
        f"Unknown LLM provider '{config.provider}'. "
        "Use 'deepseek', 'anthropic', or 'azure-openai'."
    )
...
class LLMCouncil:
    ...
    By default orchestrates debates between two DeepSeek models (chat + reasoner)
    via the OpenAI-compatible API. Override ``claude_config`` / ``gpt_config``
    with ``ModelConfig(provider='anthropic', ...)`` or Azure OpenAI to use
    other backends.
    """

    # Default: primary slot (historically "Claude") → DeepSeek Chat
    ...
    # Default: secondary slot (historically "GPT") → DeepSeek Reasoner
    ...
    # DeepSeek pricing (CNY per 1M tokens) from official pricing page.
    DEEPSEEK_PRICING_CNY_PER_1M = {
        "input_cache_hit": 0.2,
    ...
    def __init__(
        ...
        Args:
            claude_config: Configuration for primary debate model (slot A)
            gpt_config: Configuration for secondary debate model (slot B)
        ...
        # Initialize adapters with cache (provider selects Anthropic / DeepSeek / Azure)
        self.claude_adapter = model_adapter_for_config(self.claude_config, self.cache)
        self.gpt_adapter = model_adapter_for_config(self.gpt_config, self.cache)
    ...
    async def debate(
        ...
        result = DebateResult(
            ...
            metadata={
                ...
                "model_stats": cost_info["models"],
                "pricing": {
                    "provider": "deepseek",
                    "cny_to_usd_rate": cost_info["cny_to_usd_rate"],
                    "cny_per_1m_tokens": cost_info["pricing_cny_per_1m"],
                },
            }
        )
        ...
        self.logger.info(
            ...
            f"cost={total_cost_cny:.4f} CNY (${total_cost_usd:.4f})"
        )
...
"""
    ...
    def _build_messages(
        ...
        for round_data in history:
            peer_a = self.claude_config.name
            peer_b = self.gpt_config.name
            if perspective == "claude":
                ...
                messages.append({
                    "role": "user",
                    "content": (
                        f"[{peer_b}'s perspective]:\n{round_data.gpt_response}\n\n"
                        f"Please continue the debate, addressing the other model's points."
                    )
                })
            else:
                messages.append({
...
## Divergent Opinions
- Area 1: First model says X, second model says Y
...
"""
    ...
    def _build_synthesis_prompt(
        ...
        history_text = ""
        label_a = self.claude_config.name
        label_b = self.gpt_config.name
        for round_data in history:
            history_text += f"""
...
"""
    ...
    def _sanitize_model_dir_name(model_name: str) -> str:
        """Create a safe directory name based on model identifier."""
        sanitized = re.sub(r"[^a-zA-Z0-9_.-]+", "-", model_name.strip())
        sanitized = re.sub(r"-{2,}", "-", sanitized).strip("-")
        return sanitized or "model"

    def _compute_costs(self) -> Dict[str, Any]:
        """Compute per-model and total costs from token usage stats."""
        model_stats = {
            "primary": self.claude_adapter.get_stats(),
            "secondary": self.gpt_adapter.get_stats(),
        }
        total_cny = 0.0

        for key in ("primary", "secondary"):
            stats = model_stats[key]
            provider = (stats.get("provider") or "").lower()
            ...
            cached_prompt_tokens = int(stats.get("cached_prompt_tokens", 0) or 0)
            uncached_prompt_tokens = max(0, prompt_tokens - cached_prompt_tokens)

            cost_cny = None
            if provider == "deepseek":
                p = self.DEEPSEEK_PRICING_CNY_PER_1M
                cost_cny = (
                    (cached_prompt_tokens / 1_000_000.0) * p["input_cache_hit"] +
                    (uncached_prompt_tokens / 1_000_000.0) * p["input_cache_miss"] +
                    (completion_tokens / 1_000_000.0) * p["output"]
                )
                total_cny += cost_cny

            stats["uncached_prompt_tokens"] = uncached_prompt_tokens
            stats["estimated_cost_cny"] = cost_cny
            stats["estimated_cost_usd"] = (
                (cost_cny / self.CNY_TO_USD_RATE) if cost_cny is not None else None
            )
            stats["artifact_dir_name"] = self._sanitize_model_dir_name(str(model_name))

        total_usd = total_cny / self.CNY_TO_USD_RATE
        return {
            "models": model_stats,
            "total_cost_cny": total_cny,
            "total_cost_usd": total_usd,
            "cny_to_usd_rate": self.CNY_TO_USD_RATE,
            "pricing_cny_per_1m": self.DEEPSEEK_PRICING_CNY_PER_1M,
        }
    ...
    def _create_artifact_directory(self) -> Path:
        ...
        # Create model-aligned subdirectories
        primary_dir = self._sanitize_model_dir_name(self.claude_config.name)
        secondary_dir = self._sanitize_model_dir_name(self.gpt_config.name)
        (artifact_path / primary_dir).mkdir(parents=True, exist_ok=True)
        (artifact_path / secondary_dir).mkdir(parents=True, exist_ok=True)
    ...
    def _save_round_artifacts(
        ...
        # Save responses under model-aligned folders
        primary_dir = self._sanitize_model_dir_name(self.claude_config.name)
        secondary_dir = self._sanitize_model_dir_name(self.gpt_config.name)
        claude_file = artifact_path / primary_dir / f"round{round_data.round_number}.txt"
        claude_file.write_text(round_data.claude_response, encoding="utf-8")

        gpt_file = artifact_path / secondary_dir / f"round{round_data.round_number}.txt"
...
- Estimated Cost: {result.total_cost_cny:.4f} CNY (${result.total_cost_usd:.4f})
...
async def main():
    ...
    print(f"Cost: {result.total_cost_cny:.4f} CNY (${result.total_cost_usd:.4f})")
    print("\n" + "-" * 60)
...
def run_sync_cli() -> None:
    """Synchronous entry point for setuptools console script ``automotive-llm-council``."""
    asyncio.run(main())
